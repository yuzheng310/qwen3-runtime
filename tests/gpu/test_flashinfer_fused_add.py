...
from qwen3_runtime.layers.ops import Ops, torch_fused_add_rmsnorm
...
def test_fused_add_rmsnorm_matches_add_then_rmsnorm():
    ...
    want_x, want_res = torch_fused_add_rmsnorm(x.clone(), residual.clone(), weight, eps)
    got_x, got_res = Ops.flashinfer().fused_add_rmsnorm(x.clone(), residual.clone(), weight, eps)
    torch.testing.assert_close(got_res, want_res, atol=0, rtol=0)
    torch.testing.assert_close(got_x, want_x, atol=2e-2, rtol=2e-2)
